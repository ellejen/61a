...
def roll_dice(num_rolls, dice=six_sided):
    ...
    for x in range(num_rolls):
        ...
        calldice += 1
        if roll == 1 :
            for x in range(num_rolls - calldice):
                dice()
            return 1
        else:
            sumroll += roll
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    """
    who = 0  
    commentary_0 = announce_highest(who)
    
    who = other(who)
    commentary_1 = announce_highest(who)
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    def say(score0, score1):
        ...
        if (score - last_score) > running_high:
            current_high = score - last_score
            print(current_high, 'point(s)! The most yet for Player', who)    
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    roll_best = make_averaged(roll_dice, trials_count)
    
    hold_best = 1
    best_score = roll_best(1, dice)
    max_score = hold_best
    ...
    while count <= 10:
        ...
        max_score = roll_best(count, dice)
...
def extra_turn_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 11
    zero_score = score + free_bacon(opponent_score)
    if swine_align(zero_score, opponent_score) or pig_pass(zero_score, opponent_score):
        return 0
